# Active Planning Phases

This folder holds active planning phases for the concept-reset refactor sequence.

## Current Authority

The current active planning authority is:

```text
Phase 002 — Concept-to-Architecture Refactor Roadmap
```

Phase 001 completed concept validation and refactor readiness.

Older v2.1 numeric phase files remain in this folder as reference material until they are reconciled, archived, or superseded during Phase 002.

---

# Active Phase 002 Sequence

| File | Status | Role |
|---|---|---|
| [002_concept_to_architecture_refactor_roadmap.md](002_concept_to_architecture_refactor_roadmap.md) | Active | Phase 002 overview and sequence |
| [002a_documentation_authority_terminology_inventory.md](002a_documentation_authority_terminology_inventory.md) | Complete | Documentation authority and terminology inventory |
| [002b_concept_to_domain_model_mapping.md](002b_concept_to_domain_model_mapping.md) | Complete | Concept-to-domain model mapping |
| [002c_data_lifecycle_retention_architecture_plan.md](002c_data_lifecycle_retention_architecture_plan.md) | Complete | Data lifecycle and retention architecture plan |
| [002d_privacy_boundary_encryption_architecture_plan.md](002d_privacy_boundary_encryption_architecture_plan.md) | Complete | Privacy boundary and encryption architecture plan |
| 002e_analysis_boundary_hypothesis_safety_architecture_plan.md | Next | Analysis boundary, hypothesis, and safety architecture plan |
| 002f_cost_state_personal_deployment_architecture_plan.md | Planned | Cost state and personal deployment architecture plan |
| 002g_ui_ux_concept_alignment_plan.md | Planned | UI/UX concept alignment plan |
| 002h_refactor_backlog_sequencing_acceptance_gates.md | Planned | Refactor backlog, sequencing, and acceptance gates |
| 002i_phase_exit_review_consolidation.md | Mandatory gate | Phase 002 exit review and consolidation |

---

# Mandatory Exit Gate

Every numbered phase must end with an exit review and consolidation subgroup.

For Phase 002, the required final subgroup is:

```text
002-I — Phase 002 Exit Review and Consolidation
```

Phase 003 may not start until 002-I is complete.

---

# Non-Goals

Do not start implementation refactoring from this folder until Phase 002 exits.

Do not restore GitHub Actions until the new pipeline approach is explicitly planned.
