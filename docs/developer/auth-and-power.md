...
> Documents the v2.1 auth/power deploy path and the wake handoff cookie exchange (PR #18). Verified against `backend/api/routes/auth.py`, `power.py`, `infra/lambda/power_control/handler.py`, `frontend-react/src/components/AppShell.tsx`, and deploy/pause workflows.
...
### Wake handoff (session cookie exchange)

Handoff tokens are HMAC-signed with `POWER_HANDOFF_SECRET` (TTL ~15 minutes). They bridge asleep Lambda auth to an awake API session.

**Happy path (Lambda `/login` HTML):**

1. User completes OTP on `/login` while the stack is asleep/waking.
2. `POST /api/v1/ops/power/auth/verify-code` returns `handoff_token`; Lambda stores it in `sessionStorage` (not the URL).
3. Page polls `GET /api/v1/ops/power/status` until state is `awake` (accepts Dynamo `state` or verify payload `status`).
4. Lambda `POST`s `/api/v1/ops/power/handoff` with `{ "token": "<handoff_token>" }` and `credentials: 'same-origin'`.
5. API validates the token, mints a Postgres `UserSessionRow`, sets HTTP-only `rre_session`, touches activity, and returns the user.
6. Lambda clears `sessionStorage` and navigates to `/` **without** putting the token in the query string.

**Why not `/?handoff=`:** ALB always serves `/login` from Lambda. If the token only lived in the URL and nothing exchanged it for a cookie, React `AppShell` (with `SESSION_AUTH_REQUIRED`) bounced unsigned users back to `/login` forever.

**Defense in depth:** `AppShell` still consumes a legacy `?handoff=` query param via `api.powerHandoff`, strips it from the URL, then refetches `/auth/me`. Primary path is Lambda → cookie → `/`.

Constraint: `POWER_HANDOFF_SECRET` must match between Lambda env and the API task. Coverage: `tests/test_auth_rbac_and_power.py` (`test_lambda_login_html_exchanges_handoff_for_session`).
...
| Infinite `/login` loop after wake | Handoff never exchanged for cookie (pre-PR #18 bug), or handoff `POST` failing | Confirm Lambda `LOGIN_HTML` posts `/handoff` (not `?handoff=`); API reachable; browser has `rre_session` |
| “Sign-in handoff failed. Waiting for API…” | API not ready yet, or handoff rejected | Keep `/login` open until ECS healthy; retry OTP if TTL expired |
...
- AWS architecture: [aws-deployment.md](aws-deployment.md)
- CI workflows (Deploy / Pause): [ci-workflows.md](ci-workflows.md)
