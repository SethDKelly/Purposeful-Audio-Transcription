...
| Mode | How |
|------|-----|
| Session (product) | HTTP-only cookie `rre_session` after email OTP (`/api/v1/auth/*`) — v2.1 / PR #16 |
...
| `POST` | `/api/v1/ops/power/handoff` | Exchange wake `handoff_token` → session cookie |

When the stack is **asleep**, ALB routes `/login` and `/api/v1/ops/power/auth/*` to Lambda (not the API process). See [auth-and-power.md](auth-and-power.md).
