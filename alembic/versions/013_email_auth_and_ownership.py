...
def upgrade() -> None:
    ...
    if not _table_exists("login_codes"):
        op.create_table(
            "login_codes",
            sa.Column("id", sa.String(length=36), primary_key=True),
            sa.Column("email", sa.String(length=320), nullable=False),
    ...
    if not _index_exists("ix_login_codes_email"):
    ...
    if not _index_exists("ix_user_sessions_user_id"):
        op.create_index("ix_user_sessions_user_id", "user_sessions", ["user_id"])
    if not _index_exists("ix_user_sessions_session_token_hash"):
        op.create_index(
    ...
    if not _index_exists("ix_auth_audit_events_actor_user_id"):
        op.create_index(
    ...
    for table in (
        ...
        if not _column_exists(table, "owner_user_id"):
            op.add_column(
                table,
