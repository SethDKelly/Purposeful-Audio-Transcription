...
def upgrade() -> None:
    ...
    conn = op.get_bind()
    transcripts = conn.execute(sa.text("SELECT id, created_at, source_type FROM transcripts")).fetchall()
    for transcript_id, created_at, source_type in transcripts:
        ...
        conn.execute(
            sa.text(
                "UPDATE transcripts SET current_version_id = :version_id WHERE id = :transcript_id"
            ),
    ...
    # SQLite cannot easily ALTER to NOT NULL + unique; enforce via app + unique index when possible.
