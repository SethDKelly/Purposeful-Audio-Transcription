...
- **Power Lambda env (`local.power_lambda_env` in `power.tf`):** `POWER_STATE_TABLE`, `SES_FROM_EMAIL`, `POWER_HANDOFF_SECRET`, `CODEBUILD_PROJECT_NAME`, `NAME_PREFIX` (plus per-function `API_BASE_URL` / idle `API_KEY`). **Do not set `AWS_REGION` or `AWS_DEFAULT_REGION`** on these Lambdas — both are reserved by the runtime and Terraform create fails with a reserved-key error. Handlers read the runtime-injected values (fallback `us-east-2`). ECS tasks and CodeBuild may still set region env vars.
